(function($){
	...
	$.fn.tourmaster_datepicker = function(){
		...
		$(this).each(function(){
			...
			} else if(!$(this).attr('data-dob-date')) {
				$(this).datepicker({
					maxDate : '0',
					dateFormat: date_format,
					altFormat: 'yy-mm-dd',
	...
	// tourmaster_dob_datepicker
	$.fn.tourmaster_dob_datepicker = function(){

		$(this).each(function(){
			var alternate_field = $(this).siblings('.tourmaster-dob-datepicker-alt');
	...
	tourmaster_payment_template.prototype = {
		...
		init: function(){
			...
			t.form.on('click', '.tourmaster-payment-step', function(){
				...
				if( $(this).attr('data-action') ){
					if( t.check_required_field(booking_detail_data,booking_detail_data['step']) ){
						t.change_step({
			...
			// bind the change state button
			// t.payment_step_back.on('click','#go-back-button', function(){
		...
		bind_script: function(){
			...
			t.sidebar.on('click', '.tourmaster-tour-booking-bar-coupon-validate', function(){
				...
				$.ajax({
					...
					data: { 'coupon_code': coupon_code.val(), 'coupon_type': 'coupon', 'tour_id': $(this).attr('data-tour-id'), 'action': 'tourmaster_validate_coupon_code' },
					dataType: 'json',
					beforeSend: function(){
			...
			t.sidebar.on('click', '.tour_seva', function(e){
				e.preventDefault();
				var payment_template_wrapper = $("#tourmaster-payment-template-wrapper")[0];
				if( $(this).is(':checked') ){
					var seva_ids = null;
					var booking_details = JSON.parse(payment_template_wrapper.getAttribute("data-booking-detail"));
					// booking_details['seva-ids'] = booking_details['seva-ids'].filter(Boolean);
					var serva_id = this.attributes[2].value;
					if(null != booking_details['seva-ids'] && (booking_details['seva-ids'].length > 0)){
						seva_ids = booking_details['seva-ids'];
						if(seva_ids.indexOf(serva_id) === -1){
							seva_ids.push(serva_id);
						}

					}else{
						seva_ids = [this.attributes[2].value];
					}

					booking_details["seva-ids"] = seva_ids;
					booking_details["updated_seva"] = true;
					payment_template_wrapper.setAttribute("data-booking-detail", JSON.stringify(booking_details));
					t.change_step({ booking_detail: booking_details, sub_action: 'update_sidebar' });
					...
					var booking_details = JSON.parse(payment_template_wrapper.getAttribute("data-booking-detail"));
					// booking_details['seva-ids'] = booking_details['seva-ids'].filter(Boolean);
					var serva_id = $(this).attr('id').replace("seva-","");
					// var serva_id = this.attributes[2].value;
					if((booking_details['seva-ids'].length > 0)){
						seva_ids = booking_details['seva-ids'];
						if(null != seva_ids.indexOf(serva_id) && seva_ids.indexOf(serva_id) > -1){

							seva_ids.splice(seva_ids.indexOf(serva_id),1);
			...
			t.content.on('click', '#tourmaster-payment-billing-copy', function(){
				if( $(this).is(':checked') ){
					...
					// var contact_info = billing_info.parent().parent().parent().siblings().next().find('.tourmaster-payment-contact-wrap');
					var contact_info = billing_info.parent().parent().parent().prev().find('.tourmaster-payment-contact-wrap');					billing_info.find('[data-contact-detail]').each(function(){
			...
			// Submit only after checking terms and service.
			t.content.on('click', '.term-and-service', function(){
				// alert($('.term-and-service:checked').length);
				if($('.term-and-service:checked').length == $('.term-and-service').length) {
					$('#takeme-to-payment-gateway').find('*').attr('disabled', false);
					$('#t-and-c-warning').hide();
				} else {
					$('#takeme-to-payment-gateway').find('*').attr('disabled', true);
					$('#t-and-c-warning').show();
				}
			});
			...
			// 		var billing_info = $(this).closest('.tourmaster-payment-billing-wrap');
			// 		// var contact_info = billing_info.siblings('.tourmaster-payment-contact-wrap');
			// 		var contact_info = billing_info.parent().parent().parent().siblings()[1].children[1].children[1].children[0];
			//
			...
			t.content.on('click', '[data-method]', function(){
				if( t.check_required_field(null,5) ){
					...
					}else if( $(this).attr('data-method') == 'pay' ){

						var booking_detail_data = t.get_booking_detail();
						booking_detail_data["payment-method"] = "instamojo";

						var price = $(this).attr('data-price');
						t.change_step({
							'action': action,
							'type': type,
							'pilgrim_amount': price,
							'step': 5,
							'booking_detail': booking_detail_data
						});
					}
			...
			t.content.on('click', '.iqhired-payment-plugin-complete', function(){
				t.change_step({
		...
		check_required_field: function(booking_detail_data,step){

			var $terms_links = $('#tourmaster-payment-terms .tands_btn').click(function () {
			...
			// if( error_box.length ){
				...
				if(step == 2) {
					...
					//error_box = t.form.find('.tourmaster-pilgrim-details-required-error');
					var error_txt = "";
					var errors = [];
					var voucher_codes = [];
					var defVal = ["DOB" , "-1"];
					var parsedData = JSON.parse(t.form[0].dataset.bookingDetail);
					var traveller_count = 0;
					t.form.find('input[data-required], select[data-required], textarea[data-required] , input[validate-coupon] ,input[validate-phone],input[validate-econtact-phone]').each(function () {

						// $(this).removeClass('tourmaster-validate-error');
						// if ((this.name == "traveller_voucher[]") && (!isNullOrWhiteSpace(this.value))){
						// 	validateVoucherCode(this.value ,'voucher', booking_detail_data["tour-id"] ,voucher_codes, errors,$(this).attr('data-ajax-url'));
						// 	//$booking_detail['voucher-codes']
						// 	console.log('validateVoucherCode errors',errors);
						// 	error = 'voucher';
						// 	if( errors.length > 0 ){
						...
						$(this).removeClass('tourmaster-validate-error');
						if (!(this.name == "traveller_voucher[]") && !(this.name == "traveller_phone[]")
							&& !(this.name == "emergency_contact_phone_no") && (!this.value || (defVal.indexOf(this.value) > -1))) {
							$(this).addClass('tourmaster-validate-error');
							error = 'default';
							errors.push(error);
						}else if ((this.name == "traveller_first_name[]") && (isNullOrWhiteSpace(this.value))) {
							$(this).addClass('tourmaster-validate-error');
							this.setCustomValidity('Enter valid Name');
							error = 'firstname';
							errors.push(error);
						}else if ((this.name == "emergency_contact_name") && (isNullOrWhiteSpace(this.value))) {
							$(this).addClass('tourmaster-validate-error');
							this.setCustomValidity('Enter valid Name');
							error = 'econtact-name';
							errors.push(error);
						}else if ((this.name == "traveller_phone[]")) {
							$(this).addClass('tourmaster-validate-error');
							this.setCustomValidity('Enter valid Phone Number');
							var filter = /((\+*)((0[ -]+)*|(91 )*)(\d{12}|\d{10}))|\d{5}([- ]*)\d{6}/;
							// if (!filter.test(parsedData.traveller_id_no)) {
							if (!filter.test(this.value)) {
								error = 'phone-no';
								errors.push(error);
								$(this).addClass('tourmaster-validate-error');
							}
						}else if ((this.name == "emergency_contact_phone_no")) {
							$(this).addClass('tourmaster-validate-error');
							this.setCustomValidity('Enter valid Phone Number');
							var filter = /((\+*)((0[ -]+)*|(91 )*)(\d{12}|\d{10}))|\d{5}([- ]*)\d{6}/;
							// if (!filter.test(parsedData.traveller_id_no)) {
							if (!filter.test(this.value)) {
								error = 'econtact-phone-no';
								errors.push(error);
								$(this).addClass('tourmaster-validate-error');
							}
						}else if ((this.name == "DOB[]") && (this.value == "DOB")) {
							$(this).addClass('tourmaster-validate-error');
							this.setCustomValidity('Select Date of Birth');
							error = 'dob';
							errors.push(error);
						}else if ((this.name == "tour_travellers_id[]") && (this.value == "-1")) {
							$(this).addClass('tourmaster-validate-error');
							this.setCustomValidity('Select ID Type');
							error = 'idtype';
							errors.push(error);
						}else if ((this.name == "gender[]") && (this.value == "-1")) {
							$(this).addClass('tourmaster-validate-error');
							this.setCustomValidity('Select Gender Type');
							error = 'gender';
							errors.push(error);
						}else if ((this.name == "traveller_id_no[]") && (!isNullOrWhiteSpace(this.value))) {
							var id_type = parsedData.tour_travellers_id[0];
							parsedData.tour_travellers_id = parsedData.tour_travellers_id.slice(1);

							if(id_type.match(/aadhaar/i)){
							...
							}else if(id_type.match(/passport/i)){
								var filter = /^[A-Z][1-9]\d\s?\d{4}[1-9]$/igm;
								if (!filter.test(this.value)) {
									error = 'passport';
									errors.push(error);
									$(this).addClass('tourmaster-validate-error');
								...
								var filter = /^[a-zA-Z]{2}[a-zA-Z0-9]+$/igm;
								if (!filter.test(this.value)) {
									error = 'invalidchar';
									errors.push(error);
									$(this).addClass('tourmaster-validate-error');
						...
						} else if ((this.name == "traveller_voucher[]") && (!isNullOrWhiteSpace(this.value))){
							validateVoucherCode(this.value ,'voucher', booking_detail_data["tour-id"] ,voucher_codes, errors,$(this).attr('data-ajax-url'));
							//$booking_detail['voucher-codes']
							// console.log('validateVoucherCode errors',errors);
					...
					if( errors.length === 0 ){
						t.form.find('input[validate-file]').each(function () {
							// alert('Inside find 1599');
							// var today = new Date();
							// var birth = new Date(booking_detail_data["DOB"][traveller_count]);
							...
							// 	age = parseInt(age) -1;
							// }
							// booking_detail_data["age"][traveller_count] = age;

							$(this).removeClass('tourmaster-validate-error');
							if (this.name == "fileToUpload[]"){
								// alert('Inside if 1620');
								...
								for (var i = 0; i < files.length; i++) {
									...
									var file = files[i];
									var fileName = booking_detail_data["tour-date"]+"/"+booking_detail_data["tour-id"]+"_"+ parsedData["mem-id"][traveller_count] +"_" + new Date().getTime();

									// file.setnam = fileName;
									formData.append('files[]', file);
									formData.append("name[]", fileName);
									formData.append("upload_file", true);
									uploadUserIDFile(formData, errors);
									booking_detail_data["fileToUpload"][traveller_count] = fileName + file.name.slice(file.name.lastIndexOf('.'));
									traveller_count = traveller_count + 1;
								}
					...
					if( errors.length > 0 ){
						...
						var errs = Array.from(new Set(errors));
						for (var i = 0; i < errs.length; i++) {
							error_txt +=  error_box.data(errs[i]) + "<br>";
						}
					}
					...
					if (!error) {
						//alert('inside !error');
						$(this).removeClass('tourmaster-validate-error');
						booking_detail_data['voucher-codes'] = voucher_codes;

					}else{
						$(this).addClass('tourmaster-validate-error');
						error_box.html(error_txt);
						error_box.slideDown(200);
						// $('html, body').animate({scrollTop: scrollPos}, 200, 'easeOutQuad');

						var scrollPos = error_box.offset().top - $(window).height() + 600;
						//scrollPos = error_box.offset().top - $(window).height() + 150;
						if( scrollPos > 0 ){
						}
					}
					// exit();
				...
				if(step == 4){
					t.form.find('input[data-required], select[data-required], textarea[data-required]').each(function(){
						if(this.localName == "select" &&  this.value < 0){
							$(this).addClass('tourmaster-validate-error');
							error = 'default';
						}else if( !$(this).val() ){
							$(this).addClass('tourmaster-validate-error');
					...
					if( error ){
						$('#gdlr-accordion').children().find('.gdlr-core-accordion-item-content').addClass('gdlr-core-active gdlr-core-active-display');
						$( "a[id^='gdlr-core-accordion-next-button']" ).addClass('display_none');
						//$('#gdlr-core-accordion-next-button').addClass('display_none');
				...
				if( step == 5 ){ //tourmaster-payment-terms
					...
					t.form.find('[name="tourmaster-payment-terms"]').each(function(){

						// if( !$terms_links.is('.clicked')){
						// 	error = 'default';
						// 	error_box.html(error_box.data(error));
						// 	error_box.slideDown(200);
						// }
		...
		change_step: function(ajax_data, ajax_settings){
			...
			var ajax_data1 = $.extend({
				action: 'tourmaster_payment_template',
			}, ajax_data);

			var ajax_settings1 = $.extend({
				beforeSend: function(){
				...
				success: function( data ){
					...
					if( typeof(ajax_data.booking_detail) != 'undefined' || typeof(ajax_data.step) != 'undefined' ){
						...
						if( typeof(ajax_data.step) != 'undefined' ){
							booking_step = ajax_data.step;
						} else if( typeof(ajax_data.booking_detail) != 'undefined' ){
							booking_step = ajax_data.booking_detail.step;
						}
			...
			$.ajax({
				...
				data: (ajax_data1),
				dataType: 'json',
				beforeSend: ajax_settings1.beforeSend,
				error: ajax_settings1.error,
				success: ajax_settings1.success
			});
	...
	$(document).ready(function(){
		...
		// trigger the datepicker
		$('.tourmaster-dob-datepicker').tourmaster_dob_datepicker();

		// trigger the datepicker
		// $('.tourmaster-child-dob-datepicker').tourmaster_child_dob_datepicker();

		// trigger the datepicker
		// $('.tourmaster-sr-citizen-dob-datepicker').tourmaster_sr_citizen_dob_datepicker();

		// trigger the datepicker
		// $('.tourmaster-adult-dob-datepicker').tourmaster_adult_dob_datepicker();
	...
	function validateVoucherCode(coupon_code , coupon_type, tourID  , voucher_codes,errors,url) {

		$.ajax({
			type: 'POST',
			url: url,
			async: false,
			data: { 'coupon_code': coupon_code, 'coupon_type': coupon_type, 'tour_id': tourID , 'action': 'tourmaster_validate_coupon_code' },
			dataType: 'json',
			error: function( jqXHR, textStatus, errorThrown ){
			...
			success: function( data ){
				...
				// console.log('validateVoucherCode data',data);
				// alert(data.status);
				if(data.status == "failed"){
					errors.push("voucher");
				}else{
					if (voucher_codes.indexOf(coupon_code.trim()) != "-1"){
						errors.push("voucher-duplicate");
					}else{
						voucher_codes.push(coupon_code.trim());
					}
	...
	function uploadUserIDFile(formData ,errors) {
		// alert('inside uploadUserIDFile');
		$.ajax({
			type: 'POST',
			url: '/wp-content/plugins/pilgrimagetourmaster/include/fileUpload.php',
			async: false,
			data: formData,
			processData: false,
			contentType: false,
			...
			success: function( data ){
				...
				// console.log('uploadUserIDFile success data',data);
				if(resp[0].status == "failed"){
					errors.push(resp[1].errorCode);
				}
			}
		});
		// console.log('uploadUserIDFile errors',errors);
	...
	$(document).ready(function(){
		...
		$( "input[id^='id-upload']" ).change(function () {
			var index = this.id.substr(10);
			var file_sz_place = 'fp-' + index;
			if (this.files.length > 0) {

				$.each(this.files, function (index, value) {
					var label_text = $('.tourmaster-file-label-text')[index];
					if(label_text != null){
						//label_text.attributes[1].value = Math.round(value.size / 1024) + 'KB';
						document.getElementById(file_sz_place).innerHTML = "ID File Size - " + Math.round(value.size / 1024) + 'KB';
						//label_text.html( Math.round((value.size / 1024)));
					}else{
						//label_text.html(label_text.attr('data-default'));
					}
				})
		...
		//$('#gdlr-accordion').accordion();
	});

	$('body').on('click','#second-step-backbutton2',function(){
		$('#pilgrim-contact-details-title').click();
	});
	$('body').on('click','#second-step-backbutton3',function(){
		$('#payee-details-title').click();
	});

	// $(document).
	$('body').on('click','.tourmaster-payment-contact-title',function(){
	...
	$('body').on('click','.tourmaster-payment-billing-title',function(){
		$(this).siblings('.gdlr-core-accordion-item-content').slideDown(200);
	...
	$('body').on('click','.gdlr-core-accordion-next-button',function(){

		var error = false;
		var index = this.id.slice(32);
		var classN = 'tourmaster-tour-booking-required-error-' + index;
		var error_box = $("div[class^="+classN+"]")
		var error_txt = "";
		var errors = [];
		if( error_box.length ) {
			var index = error_box[0].className.substr(39);
			error_box.slideUp(200);

			var item_tab = $(this).closest('.gdlr-core-accordion-item-tab');

			item_tab.find('input[data-required], select[data-required]').each(function () {
				$(this).removeClass('tourmaster-validate-error');
				if (!$(this).val()) {
					$(this).addClass('tourmaster-validate-error');
					error = 'default';
					errors.push(error);
				}else if (this.localName == "select" && this.value < 0) {
					$(this).addClass('tourmaster-validate-error');
					error = 'default';
					errors.push(error);
				} else if(this.localName == "input" && this.name == "DOB" && this.value == "Select DOB"){
					$(this).addClass('tourmaster-validate-error');
					this.setCustomValidity('Invalid Date');
					error = 'default';
					errors.push(error);
				}else if(this.localName == "input" && (this.name == "email") || this.name == "billing_email"){
				...
				}else if(this.localName == "input" && (this.name == "phone" || this.name == "billing_phone")){
					// var filter = /((\+*)((0[ -]+)*|(91 )*)(\d{12}\+|\d{10}\+))|\d{5}([- ]*)\d{6}/;
					var filter = /\+?\d[\d -]{8,12}\d/;
					if (!filter.test(this.value)) {
						error = 'phone';
						errors.push(error);
						$(this).addClass('tourmaster-validate-error');
					}
				}else if(this.localName == "input" && this.name == "traveller_id_no"){

					var id_type = this.previousSibling.selectedOptions[0].textContent;
			...
			if (!error) {
				$(this).removeClass('tourmaster-validate-error');
				if( item_tab.hasClass('gdlr-core-active') ){
					var next_tab = item_tab.next();
					if( typeof(next_tab) != 'undefined' ){
						next_tab.addClass('gdlr-core-active').removeClass('gdlr-core-inactive')
							.find('.gdlr-core-accordion-item-content').css({display: 'block'}).slideDown(200);
					}
					// item_tab.removeClass('gdlr-core-active');
					item_tab.removeClass('gdlr-core-active')
						.find('.gdlr-core-accordion-item-content').css({display: 'block'}).slideUp(200);
				...
				//var scrollPos = error_box.offset().top - $(window).height() + 600;
				var scrollPos = error_box.offset().top - $(window).height() + 150;
				if( scrollPos > 0 ){
					$('html, body').animate({scrollTop: scrollPos}, 200, 'easeOutQuad');
				}
			}
	...
	$(window).load(function() {
		...
		// input file
		// $('.tourmaster-file-label').on('change', 'input[type="file"]', function(){
		// 	var label_text = $(this).siblings('.tourmaster-file-label-text');
		//
		// 	if( $(this).val() ){
		// 		label_text.html($(this).val().split('\\').pop());
