...
//		'departed' => esc_html__('Departed', 'tourmaster'),
    ...
    if( !empty($result->payment_info) ){
        ...
        if(array_key_exists("error",$payment_info)){
            echo '<div class="tourmaster-my-booking-single-field clearfix" >';
            echo '<span class="tourmaster-head">' . esc_html__('Pending Payment Amount', 'tourmaster') . ' :</span> ';
            echo '<span class="tourmaster-tail">' . tourmaster_money_format($result->total_price ) . '</span>';
            echo '</div>';
        }else if( !empty($payment_info['payment_mode']) && $payment_info['payment_mode'] == "Payment Failed" ){
            $dep_amt = ($payment_info['deposit_amount'] == null)?0:$payment_info['deposit_amount'];
            $totpayment_amt = $result->total_price;
            echo '<div class="tourmaster-my-booking-single-field clearfix" >';
            echo '<span class="tourmaster-head">' . esc_html__('Pending Payment Amount', 'tourmaster') . ' :</span> ';
            echo '<span class="tourmaster-tail">' . tourmaster_money_format($totpayment_amt - $dep_amt) . '</span>';
            echo '</div>';
        }
        ...
        echo '<div class="tourmaster-my-booking-single-field clearfix" >';
        echo '<span class="tourmaster-head">' . esc_html__('Pending Payment Amount', 'tourmaster') . ' :</span> ';
        echo '<span class="tourmaster-tail">' . tourmaster_money_format($totpayment_amt) . '</span>';
        echo '</div>';
    }
	if( in_array($result->order_status, array('pending', 'rejected', 'receipt-submitted')) ){
		...
		echo '<a href="';
		echo esc_url(add_query_arg(array('tid'=>$result->id, 'step'=>4), tourmaster_get_template_url('payment')));
	...
	if( !empty($result->traveller_info) ){
		...
		if( !empty($traveller_info) ){
			...
			echo '<h3 class="tourmaster-my-booking-single-title">' . esc_html__('Pligrim Info', 'tourmaster') . '</h3>';

			$name_array = $traveller_info[0]["traveller_first_name"];
			$age_array = $traveller_info[1]["age"];
			$id_type_array = $traveller_info[2]["tour_travellers_id"];
			$id_val_array = $traveller_info[3]["traveller_id_no"];
			$id_file_array = $traveller_info[4]["fileToUpload"];
            $current_url = (is_ssl()? "https": "http") . "://" . $_SERVER['HTTP_HOST'];
			$path = $current_url.'/wp-content/uploads/documents/';
...
//			}
			for( $i=0; $i<sizeof($name_array); $i++ ){
				if( !empty($name_array[$i])){
					...
					echo '<span class="tourmaster-tail">';
                    echo '<span class="">' . ($i+1) . ' :</span> ';
                    echo '<span class="tourmaster-head1">' . esc_html__('Name :', 'tourmaster') . ' </span> ' . $name_array[$i];

                    if( !empty($age_array[$i]) ){
                        echo '<span class="tourmaster-head1">' . esc_html__('Age :', 'tourmaster') . ' </span> ' . $age_array[$i];
                    }
                    if( !empty($id_type_array[$i]) ){
                        echo '<span class="tourmaster-head1">' . esc_html__('ID :', 'tourmaster') . ' </span> ' . $id_type_array[$i];
                    }
                    if( !empty($id_val_array[$i]) ){
                        echo '<span class="tourmaster-head1">' . esc_html__('ID No :', 'tourmaster') . ' </span> ' . $id_val_array[$i];
                    }
                    if( !empty($id_file_array[$i]) ){
                        echo '<span class="tourmaster-head1">' . esc_html__('Uploaded ID : ', 'tourmaster') . ' </span> ';
                        echo '<a href="' . $path .$id_file_array[$i]. '" class="strip tourmaster-head1 gdlr-core-ilightbox gdlr-core-js "><i class="fa fa-file-text-o"></i></a>';
                    }
