...
//			}
            ...
            if( !empty($_GET['payment_method']) && $_GET['payment_method'] == 'instamojo' ){
                $booking_detail['payment_method'] = 'instamojo';
            }


			$transaction_id = $_GET['tid'];
            $booking_detail['transaction_id'] = $transaction_id;
...
get_header();
	...
	if( !empty($booking_detail['tour-id']) ){
		...
		$payment_steps = array(
            ...
            esc_html__('Pilgrims', 'tourmaster'),
			esc_html__('Contact', 'tourmaster'),
//			esc_html__('Seat', 'tourmaster'),
		...
		echo '<div id= "tourmaster-payment-step-container" class="tourmaster-payment-step-container tourmaster-container" >';
		echo '<div class="tourmaster-payment-step-inner tourmaster-item-pdlr clearfix" >';
//        $enable_bus_booking = $tour_option['require-bus-booking'];
        $next_step = $booking_detail['next-step'];
        if($booking_step == 4 && $next_step== 5){
            $booking_step = 5;
        }
		foreach( $payment_steps as $payment_step ) {
            echo '<div style="pointer-events: none;" class="tourmaster-payment-step-item ';
...
//
	...
	echo '<div class="tourmaster-container tourmaster-payment-container" >';
...
do_action('include_iqhired_payment_script');
