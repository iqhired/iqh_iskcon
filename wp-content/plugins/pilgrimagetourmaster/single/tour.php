...
get_header();
		...
		if( !empty($tour_rating['reviewer']) ){
			$schema['AggregateRating'] = array(
				array(
					...
					'ratingValue' => intval($tour_rating['score']) / (2 * intval($tour_rating['reviewer'])),
	...
	echo ' >';
	if( function_exists('gdlr_core_get_lightbox_atts') && $tour_option['header-image'] == 'gallery' && !empty($tour_option['header-slider']) ){
    ...
    if( empty($tour_option['display-single-tour-info']) || $tour_option['display-single-tour-info'] == 'enable' ){
        echo '<div id="tourmaster-tour-info-data" class="tourmaster-tour-info-outer" >';
...
//        echo '</div>'; // tourmaster-tour-info-outer-container
	...
	// content section
	////////////////////////////////////////////////////////////////////
	echo '<div class="tourmaster-template-wrapper" id = "tourmaster-template-wrapper-booking-pg">';
		...
		if( $tour_option['form-settings'] == 'booking' || $tour_option['form-settings'] == 'both' ){
			...
			// normal form
				...
				if( !empty($available_date) ){
					...
					echo '<div id= "ll" class="tourmaster-tour-booking-date-input" >';
					...
					if( sizeof($available_date) == 1 ){
						...
						if( $date_selection_type == 'calendar' ){
                            ...
                            echo '<option value="' . esc_html__('Choose a date', 'tourmaster') . '" ' . 'selected' . ' >' . "Choose a date" .  '</option>';
...
//								echo '<option value="' . esc_attr($available_date_single) . '" ' . ($selected_date == $available_date_single? 'selected': '') . ' >';
								echo '<option value="' . esc_attr($available_date_single) . '" >';
					...
					if( !empty($temp_data) ){
						$booking_value = array(
                            ...
                            'tour-sr-citizen' => empty($temp_data['tour-sr-citizen'])? '': $temp_data['tour-sr-citizen'],
						...
						unset($temp_data['tour-sr-citizen']);
                    ...
                    if( !empty($date_price['package']) ){
                        $max_people = get_post_meta(get_the_ID(), 'tourmaster-max-people', true);
                        $package_group_slug = '';
                        if(!empty($date_price['package'][0])){
                            $package_group_slug = ($date_price['package'][0])['group-slug'];
                        }
                        if( !empty($max_people) ){
...
//                            ), array(), 'COUNT(*)');
                            ...
                            $ret .= '<div class="tourmaster-booking-bottom clearfix" ><div class="tourmaster-save-wish-list tourmaster-active" >';
                            $ret .= '<span class="tourmaster-save-wish-list-icon-wrap" ><i class="tourmaster-icon-active fa fa-info-circle" ></i>';
                            $ret .= '<i class="tourmaster-icon-inactive fa fa-heart-o" ></i></span>';
                            $ret .= sprintf(esc_html__('Available seats : ' , 'tourmaster'));
                            $ret .= '</div><div class="tourmaster-view-count" ><span class="tourmaster-view-count-text" >';
                            $ret .= sprintf(esc_html__(' %d ' , 'tourmaster'),($max_people - $current_amount));
                            $ret .= '</span></div></div>';
