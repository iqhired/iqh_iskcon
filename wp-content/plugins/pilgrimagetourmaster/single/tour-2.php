			...
			// normal form
				...
				if( !empty($available_date) ){
                    ...
                    if( empty($temp_data['tour-date']) ){
                        $select_date_text = esc_html__('Choose a date', 'tourmaster');
                    }else{
...
//					}
					if( sizeof($available_date) == 1 ){
						...
						if( $date_selection_type == 'calendar' ){
							...
							echo '<div id="tt"  class="tourmaster-combobox-wrap tourmaster-tour-date-combobox" >';
							echo '<select name="tt" >';
//                            echo '<option value="' . esc_html__('Choose a date', 'tourmaster') . '" ' . 'selected' . ' >';
//
