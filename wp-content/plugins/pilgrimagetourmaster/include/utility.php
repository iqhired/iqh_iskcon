<?php
	...
	*/
    ...
    if( !function_exists('getTravellersDetail') ){
        function getTravellersDetail( $traveller_info ){
            ...
            for( $i=0; $i<sizeof($name_array); $i++ ){
                if( !empty($name_array[$i])){
                    $resp  .=   ($i+1) . "      ";
                    $resp  .=  esc_html__('Name :', 'tourmaster')  . $name_array[$i]. "     ";;

                    if( !empty($age_array[$i]) ){
                        $resp  .=   esc_html__('Age :', 'tourmaster') . $age_array[$i]. "       ";;
                    }
                    if( !empty($id_type_array[$i]) ){
                        $resp  .=   esc_html__('ID :', 'tourmaster') .  $id_type_array[$i]. "       ";;
                    }
                    if( !empty($id_val_array[$i]) ){
                        $resp  .=   esc_html__('ID No :', 'tourmaster') . $id_val_array[$i]. "      ";;
                    }
                    if( !empty($traveller_info['passport'][$i]) ){
                        $resp  .=  esc_html__('Passport ID :', 'tourmaster') . ' ' . $traveller_info['passport'][$i]. "     ";;
                    }
                    if( !empty($id_file_array[$i]) ){
                        $path = $_SERVER['REQUEST_SCHEME'] . "://" . $_SERVER['HTTP_HOST'] . '/wp-content/uploads/documents/';
                        $resp  .=   esc_html__('Uploaded ID :', 'tourmaster');
//                        $resp  .=   '<a href="' . esc_url($path .$id_file_array[$i]) . '" class="strip tourmaster-head1 gdlr-core-ilightbox gdlr-core-js "> <i class="fa fa-file-text-o"></i></a>' . "      ";;
                        $resp .= esc_url($path .$id_file_array[$i])  . "      ";
                    }
                    $resp .= "\r";
                }
            }
...
//            }
			...
			switch($settings['type']){
				case 'textarea':
					...
					break;
				case 'email':
					echo '<input type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" name="' . esc_attr($settings['slug']) . '" value="' . esc_attr($field_value) . '" ' . $data . '  />';
					break;
				case 'text':
				...
				case 'dob_datepicker':
						echo '<input type="text" class="tourmaster-dob-datepicker" name="' . esc_attr($settings['slug']) . '" value="' . esc_attr($field_value) . '" />';
						echo '<i class="fa fa-calendar" ></i>';
					break;

                case 'date':
                    echo '<input type="text" style="max-width: 50% !important;margin-right: 10px;" class="tourmaster-dob-datepicker" name="' . esc_attr($settings['slug']) . '" value="' . esc_attr($field_value) . '" />';
                    echo '<i class="fa fa-calendar" ></i>';
                    break;
