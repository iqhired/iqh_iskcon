	...
	*/
	...
	if( !class_exists('tourmaster_tour_style') ){
		class tourmaster_tour_style{
            ...
            // tour price
            function get_modern_tour_price( $settings = array() ){
                ...
                if( !empty($post_meta['tour-price-text']) || !empty($post_meta['tour-price-discount-text']) ){
                    ...
                    if( !empty($post_meta['tour-price-text']) ){
                        $ret .= '<span class="tourmaster-tour-price" >';
                        $ret .= empty($settings['price-prefix-text'])? esc_html__('From  ', 'tourmaster'): $settings['price-prefix-text'];
                        $ret .= '<span class="">' . tourmaster_money_format($post_meta['tour-price-text'], 0) . '</span>';
                        $ret .= '</span>';
                    }
			...
			function get_info( $options = array(), $args = array() ){
				...
				foreach( $options as $type ){
					switch( $type ){
						...
						case 'availability': 
							if( !empty($post_meta['departure-date']) ){
								...
								$ret .= esc_html__('Departure Date :', 'tourmaster') . ' ';
								$ret .= tourmaster_text_filter($post_meta['departure-date']);
								$ret .= '</div>';
							} 
							break;
			...
			function tour_modern( $args ){
				...
				$ret .= $thumbnail;
				$ret .= '<div id = "tourmaster-tour-modern-content-wrap" class="tourmaster-tour-content-wrap" >';
				$ret .= $this->tour_title($args);
				...
				// price
				$ret .= $this->get_modern_tour_price();
