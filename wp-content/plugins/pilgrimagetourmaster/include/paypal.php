<?php
	...
	add_filter('iqhired_plugin_payment_option', 'tourmaster_paypal_payment_option');
	...
	add_filter('iqhired_paypal_payment_form', 'tourmaster_paypal_payment_form', 10, 2);
	if( !function_exists('tourmaster_paypal_payment_form') ){
		function tourmaster_paypal_payment_form( $ret = '', $tid = '' ){
			...
			$t_data = apply_filters('iqhired_payment_get_transaction_data', array(), $tid, array('price', 'tour_id'));
			...
			ob_start();
?>
<div class="iqhired-paypal-redirecting-message" ><?php esc_html_e('Please wait while we redirect you to paypal.', 'tourmaster') ?></div>
<form id="iqhired-paypal-redirection-form" method="post" action="<?php
...
<script type="text/javascript">
	(function($){
		$('#iqhired-paypal-redirection-form').submit();
	})(jQuery);
</script>
<?php
			$ret = ob_get_contents();
