<?php
	...
	*/
	...
	if( !function_exists('tourmaster_insert_booking_data') ){
		function tourmaster_insert_booking_data( $booking_detail, $tour_price, $traveller_amount = 0, $package_group_slug = '' ){
            ...
            array_push($traveller_info , array_merge(array('traveller_first_name' => $booking_detail['traveller_first_name'])));
            array_push($traveller_info , array_merge(array('age' => $booking_detail['age'])));
            array_push($traveller_info , array_merge(array('tour_travellers_id' => $booking_detail['tour_travellers_id'])));
            array_push($traveller_info , array_merge(array('traveller_id_no' => $booking_detail['traveller_id_no'])));
            array_push($traveller_info , array_merge(array('fileToUpload' => $booking_detail['fileToUpload'])));
...
//					'title' => empty($booking_detail['traveller_title'])? '': $booking_detail['traveller_title'],
					'first_name' => array($booking_detail['traveller_first_name']),
...
//					'passport' => empty($booking_detail['traveller_passport'])? '': $booking_detail['traveller_passport'],
			...
			if( empty($booking_detail['tid']) || $booking_detail['tid'] == 'false' ){
				...
				$format = array_merge(array('%d', '%d'), $format);
...
if( !function_exists('tourmaster_get_booking_trans_data') ){
    function tourmaster_get_booking_trans_data( $conditions = array(), $settings = null, $column = '*' ){
        global $wpdb;
		...
		function tourmaster_get_booking_data( $conditions = array(), $settings = null, $column = '*' ){
			...
			foreach($conditions as $condition_slug => $condition){
				...
				if( !empty($condition) && ($condition == 'IS NOT NULL' || $condition == 'IS NULL') ){
                    ...
                    if( !empty($condition['custom'])  && $condition['custom'] == "json_extract_cus"){
                        if(empty($condition['value'])) {
                            $sql =  preg_replace('/\W\w+\s*(\W*)$/', '$1', $sql)."\n";
                        }else{
                            $sql .= $condition['json_extract_query'] . '=\'' . esc_sql($condition['value']) . '\' ';
                        }
                    }else {
                        if( empty($condition['hide-prefix']) ){
                            $sql .= esc_sql($condition_slug);
