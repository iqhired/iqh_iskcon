...
function wbtm_flash_permalink_once() {
    if ( get_option( 'wbtm_flash_bus_permalink' ) != 'completed' ) {
         global $wp_rewrite;
         $wp_rewrite->flush_rules();
        update_option( 'wbtm_flash_bus_permalink', 'completed' );
    }
}
add_action( 'admin_init', 'wbtm_flash_permalink_once' );
...
global $wpdb;
  ...
  $sql = "CREATE TABLE $table_name (
    ...
    user_extra_bag int(3) NOT NULL DEFAULT 0,
...
if ( empty(get_option( 'wbtm_update_db_once_1989' )) && get_option( 'wbtm_update_db_once_1989' ) != 'completed' ) {  
    $table = $wpdb->prefix."wbtm_bus_booking_list";
    $myCustomer = $wpdb->get_row( sprintf("SELECT * FROM %s LIMIT 1", $table) );
    if(!isset($myCustomer->user_extra_bag)) {
      $wpdb->query( sprintf( "ALTER TABLE %s ADD user_extra_bag INT NOT NULL DEFAULT 0 AFTER user_address", $table) );
    }
update_option( 'wbtm_update_db_once_1989', 'completed' );
// die();
} 
...
function wbtm_page_create() {
        ...
        if (! wbtm_get_page_by_slug('bus-search-list')) {
...
function wbtm_bus_search_fileds($start,$end,$date,$r_date){
    ...
    <div class="search-fields">
      <div class="fields-li">
          <label>
             <i class="fa fa-map-marker" aria-hidden="true"></i><?php echo bus_get_option('wbtm_from_text', 'label_setting_sec') ? bus_get_option('wbtm_from_text', 'label_setting_sec') : _e('From','bus-ticket-booking-with-seat-reservation'); ?>
          <?php echo wbtm_get_bus_route_list( 'bus_start_route', $start ); ?></label>
          </div>
      <div class="fields-li">
          <label>
             <i class="fa fa-map-marker" aria-hidden="true"></i><?php echo bus_get_option('wbtm_to_text', 'label_setting_sec') ? bus_get_option('wbtm_to_text', 'label_setting_sec') : _e('To:','bus-ticket-booking-with-seat-reservation'); ?>
          <?php echo wbtm_get_bus_route_list( 'bus_end_route', $end ); ?>
          </label>
          </div>
      <div class="fields-li">
          <label for='j_date'>
            <i class="fa fa-calendar" aria-hidden="true"></i><?php echo bus_get_option('wbtm_date_of_journey_text', 'label_setting_sec') ? bus_get_option('wbtm_date_of_journey_text', 'label_setting_sec') : _e('Date of Journey:','bus-ticket-booking-with-seat-reservation'); ?>
          <input readonly type="text" id="j_date" name="j_date" value="<?php echo $date; ?>">
          </label>
          </div>
      <div class="fields-li return-date-sec">
          <label for='r_date'>
            <i class="fa fa-calendar" aria-hidden="true"></i><?php echo bus_get_option('wbtm_return_date_text', 'label_setting_sec') ? bus_get_option('wbtm_return_date_text', 'label_setting_sec') : _e('Return Date:','bus-ticket-booking-with-seat-reservation'); ?>
    ...
    <?php 
     ...
     <div class="fields-li">
       <div class="search-radio-sec">
          <label for="oneway"><input type="radio" <?php if($busr=='oneway'){ echo 'checked'; } ?> id='oneway' name="bus-r" value='oneway'><?php echo bus_get_option('wbtm_one_way_text', 'label_setting_sec') ? bus_get_option('wbtm_one_way_text', 'label_setting_sec') : _e('One Way','bus-ticket-booking-with-seat-reservation'); ?>
          </label>
          <label for="return_date"><input type="radio" <?php if($busr=='return'){ echo 'checked'; } ?> id='return_date' name="bus-r" value='return'><?php echo bus_get_option('wbtm_return_text', 'label_setting_sec') ? bus_get_option('wbtm_return_text', 'label_setting_sec') : _e('Return','bus-ticket-booking-with-seat-reservation'); ?>
          </label>
        </div>
      <button type="submit"><i class='fa fa-search'></i><?php echo bus_get_option('wbtm_search_buses_text', 'label_setting_sec') ? bus_get_option('wbtm_search_buses_text', 'label_setting_sec') : _e('Search Buses','bus-ticket-booking-with-seat-reservation'); ?>
      </button>
      </div>
    </div>
 <script>
...
// function wbtm_get_order_seat_check($bus_id,$seat,$start,$end,$date){
//   global $wpdb;
//     $table_name = $wpdb->prefix."wbtm_bus_booking_list";
//   $total_mobile_users = $wpdb->get_var( "SELECT COUNT(booking_id) FROM $table_name WHERE bus_id=$bus_id AND seat ='$seat' AND bus_start = '$start' AND journey_date='$date'" );
//   return $total_mobile_users;
// }


function wbtm_get_seat_cehck_before_order($seat,$date,$bus_id,$start){
global $wpdb;
  $table_name = $wpdb->prefix."wbtm_bus_booking_list";
$total_booking_id = 0;
// print_r($seat);
...
$total_booking = $wpdb->get_var( "SELECT COUNT(booking_id) FROM $table_name WHERE seat='$_seat' AND journey_date='$date' AND bus_id = $bus_id AND ( boarding_point ='$start' OR next_stops LIKE '%$start%' )  AND ( status = 0 OR status = 1 OR status =2 )" );
$total_booking_id = $total_booking_id + $total_booking;
}
...
function wbtm_get_seat_cehck_before_place_order($seat,$date,$bus_id,$start){
global $wpdb;
  $table_name = $wpdb->prefix."wbtm_bus_booking_list";
$total_booking_id = 0;
// print_r($seat);
...
if (get_post_type($wbtm_bus_id) == 'wbtm_bus') { 
  ...
  $extra_bag          = wbtm_get_order_meta($item_id,'Extra Bag');
  $wbtm_tp            = wbtm_get_order_meta($item_id,'_wbtm_tp');
  $seats              = explode(",",$seat);
    ...
    $add_datetime = date("Y-m-d h:i:s");
     $bi = $wpdb->insert( 
        ...
        array( 
            ...
            'user_extra_bag'  => $wbtm_extra_bag_qty,
            'bus_start'       => $b_time,
            'user_start'      => $j_time,
            'seat'            => $_seats,
            'bus_fare'        => 0,
...
$diff = round((strtotime($start_bus) - strtotime(current_time('Y-m-d H:i:s')))/3600, 1);
...
function wbtm_bus_after_remove_from_cart($cart_item_key) {
    // Your custom function
    WC()->cart->empty_cart();
}
// add_action( 'woocommerce_cart_item_removed', 'wbtm_bus_after_remove_from_cart' );



function wbtm_make_id($val){
  return  str_replace("-","",$val);

}


function wbtm_seat_booking_js($id,$fare){
?>
<script>
jQuery(document).ready(function ($) {
...
$('#bus-booking-btn<?php echo $id; ?>').hide();
    ...
    jQuery(".seat<?php echo $id; ?>_blank").on('click', function () {
        ...
        var foo         = "<tr id='selected_list<?php echo $id; ?>_" + seat<?php echo $id; ?>_name + "'><td align=center><input type='hidden' name='seat_name[]' value='" + seat<?php echo $id; ?>_name + "'/>" + seat<?php echo $id; ?>_name + "</td><td align=center><input type='hidden' name='bus_fare<?php echo $id; ?>' value=" + fare + "><?php echo get_woocommerce_currency_symbol(); ?>" + fare + "</td><td align=center><a class='button remove-seat-row<?php echo $id; ?>' data-seat='" + seat<?php echo $id; ?>_name + "'>X</a></td></tr>";
...
// alert(totalFare);
    ...
    function wbt_calculate_total(){
        ...
        jQuery('#tq<?php echo $id; ?>').val(rowCount);
        jQuery('#totalFare<?php echo $id; ?>').html("<?php echo get_woocommerce_currency_symbol(); ?>" + totalFare);
        jQuery('#tfi<?php echo $id; ?>').val(totalFare);
    if(totalFare==0){
