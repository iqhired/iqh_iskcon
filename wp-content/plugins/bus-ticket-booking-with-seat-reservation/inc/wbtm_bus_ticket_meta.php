...
function wbtm_bus_meta_box_add(){
    ...
    add_meta_box( 'wbtm-bus-operational-on-date', 'Operational On Day settings', 'wbtm_bus_on_meta_box', 'wbtm_bus', 'normal', 'high' );
...
function wbtm_bus_on_meta_box($post){
$values = get_post_custom( $post->ID );
// print_r($values);
?>
<script type="text/javascript">
  jQuery(document).ready(function( $ ){
    $( '#add-ond-row' ).on('click', function() {
      var row = $( '.empty-row-ond.screen-reader-text' ).clone(true);
      row.removeClass( 'empty-row-ond screen-reader-text' );
      row.insertBefore( '#repeatable-fieldset-ond-one tbody>tr:last' );
      return false;
    });
    $( '.remove-ond-row' ).on('click', function() {
      $(this).parents('tr').remove();
      return false;
    });
...
<p><?php _e('Operational Offday setting will not work if you set bus in particular date here.','bus-ticket-booking-with-seat-reservation'); ?></p>
<table id="repeatable-fieldset-ond-one" width="100%">
  <tr>
    <th align="left"><?php _e('Add Particular On Date','bus-ticket-booking-with-seat-reservation'); ?></th>
    <th></th>
  </tr>
  <tbody>
  <?php
  $wbtm_bus_on_dates = get_post_meta($post->ID, 'wbtm_bus_on_dates', true);
  if ( $wbtm_bus_on_dates ) :
    $coun = 0;
  foreach ( $wbtm_bus_on_dates as $field ) {
  ?>
  <tr>
    <td>
      <input type="date" name='wbtm_on_date_name[]' value="<?php if($field['wbtm_on_date_name'] != '') echo esc_attr( $field['wbtm_on_date_name'] ); ?>" class="text"></td>
    <td><a class="button remove-ond-row" href="#"><?php _e('Remove','bus-ticket-booking-with-seat-reservation'); ?></a></td>
  </tr>
  <?php
  $coun++;
  }
 ...
 ?>
  <!-- empty hidden one for jQuery -->
  <tr class="empty-row-ond screen-reader-text">
    <td><input type="date" name='wbtm_on_date_name[]' value="" class="text"></td>
    <td><a class="button remove-ond-row" href="#"><?php _e('Remove','bus-ticket-booking-with-seat-reservation'); ?></a></td>
  </tr>
  </tbody>
  </table>
  <p><a id="add-ond-row" class="button" href="#"><?php _e('Add More Date','bus-ticket-booking-with-seat-reservation'); ?></a></p>
<?php
}


add_action('save_post','wbtm_bus_on_info_save');
function wbtm_bus_on_info_save($post_id){
global $post; 
if($post){    
    $pid = $post->ID;
    if ($post->post_type != 'wbtm_bus'){
  ...
  $old = get_post_meta($post_id, 'wbtm_bus_on_dates', true);
  $new = array();
  $wbtm_on_date_name   = $_POST['wbtm_on_date_name'];
  $order_id = 0;
  $count = count( $wbtm_on_date_name );
  for ( $i = 0; $i < $count; $i++ ) {
    if ( $wbtm_on_date_name[$i] != '' ) :
      $new[$i]['wbtm_on_date_name'] = stripslashes( strip_tags( $wbtm_on_date_name[$i] ) );
      endif;
  }

  if ( !empty( $new ) && $new != $old )
    update_post_meta( $post_id, 'wbtm_bus_on_dates', $new );
  elseif ( empty($new) && $old )
    delete_post_meta( $post_id, 'wbtm_bus_on_dates', $old );
