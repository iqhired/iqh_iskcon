<?php
function wbtm_seat_global($b_start,$date){
...
?>
    <tr class="seat<?php echo get_the_id().wbtm_make_id($date); ?>_lists ">
      ...
      for ($x=1; $x <=$seatcols; $x++){
          ...
          $get_seat_status    	= wbtm_get_seat_status($_seats[$text_field_name],$date,get_the_id(),$b_start);
          if($get_seat_status) {
          ...
          <td align="center">
			<?php if($_seats[$text_field_name]){ ?>
              ...
              <?php } else { ?> 				
          	<a data-seat='<?php echo $_seats[$text_field_name]; ?>' id='seat<?php echo get_the_id().wbtm_make_id($date); ?>_<?php echo $_seats[$text_field_name]; ?>' data-sclass='Economic' class='seat<?php echo get_the_id().wbtm_make_id($date); ?>_blank blank_seat'>
